...
auth.post('/register', async (c) => {
	...
	if (!username || !password) return errorResp(c, 400, 'Username and password required');
	...
	try {
		...
		await c.env.DB.prepare(`INSERT INTO users (username, password, qq) VALUES (?, ?, ?)`)
		...
		return errorResp(c, 500, `Internal server error: ${e instanceof Error ? e.message : 'Unknown error'}`);
	}
});
